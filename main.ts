...
define(function (require, exports, module) {
  ...
  const VERSION = '3.4',
    ...
    HELPLINK = '', // deactivated for now until there is a new help page
    EXPANDTAGS = ';bu=button;d=div;sp=span;te=textarea;in=input',
    PROJSETTINGSFILE = '.projecttoix.json',
  ...
  function clipbrdCopy(text) {
    ...
    let d = document.createElement('textarea') as any;
  ...
  function clipbrdPaste() {
    let text,
      d = document.createElement('textarea') as any;
    d.contentEditable = true;
    document.body.appendChild(d);
    d.unselectable = 'off';
    d.style.position = 'absolute';
  ...
  function getCurFileName() {
    let curfile = brk.ProjectManager.getSelectedItem();
    return curfile ? curfile._path || curfile.path : '';
  }

  function getProjectRoot() {
    let root = brk.ProjectManager.getProjectRoot();
  ...
  function sendToBottomPanel(header, msgarray) {
    ...
    ix.panel.$msg.find('#cleartoix').click(function (event) {
      let $table = gettablefromevent(event);
      $table.remove();
    });
  ...
  function urlDecode() {
    changeSelection(function (text) {
      return (window as any).decodeURIComponent(text);
    }, SP_LINE);
  }
  ...
  function rgbHex() {
    replaceSelection(/(?:#([a-f0-9]{2,2})([a-f0-9]{2,2})([a-f0-9]{2,2}))|(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/ig,
      function (text, h1, h2, h3, r1, r2, r3) {
        function toHex2(v) {
          let res = parseInt(v, 10).toString(16);
  ...
  function tag() {
    changeSelection(function (text, so) {
      ...
      let id, cls, at, base, removelen, tokens;
  ...
  function replacetoix(cmd) {
    let text = getSelObj(SP_LINE).cursel.split('\n');
  ...
  function buildFuncJSDoc() {
    getSelection(function (text, so) {
      let match = text.match(/(?:(\w+)\s*[:=]\s*){0,1}function(?:\s+(\w+)){0,1}\s*\((.*?)\)/i),
  ...
  function breakLineAt(cmd) {
    ask('', cmd, ['maxcharsperline', 'tobreakwords'], function () {
      let maxchars = Math.max(1, prefs.maxcharsperline.value.trim() >> 0),
  ...
  function fullnameToClipboard() {
    copyToClipboard(function (curfile) {
      let fullname = curfile._path || curfile.path;
  ...
  function runExecute(cmd) {
    ...
    ask('', cmd, ['cmdline', 'path', 'tools', 'showoutput'], function () {
      let cmdline = fields.cmdline.value,
...
*/
  ...
  function runLanguageMapper() {
    let lang = brk.LanguageManager.getLanguage('javascript');
  ...
  function addEditCmdsToLocalMenu(ctxMenu) {
    let EDIT_CMDS = ['CMD_CUT', 'CMD_COPY', 'CMD_PASTE'],
      i, cmd, id;
  ...
  function handleAfterProjectOpen() {
    let projFile = getProjectRoot() + PROJSETTINGSFILE;
    projSettings.applied = false;
    projSettings.data = {};

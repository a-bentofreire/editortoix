...
define((require, exports, module) => {
  ...
  // @WARN: Brackets API deprecated view/PanelManager which resulted EditorToIX failed to work
  ...
  const transformutilities = require('common/transformutilities').transformutilities;
  const lineutilities = require('common/lineutilities').lineutilities;
  const um = require('common/utilitymanager').um;
    ...
    ask('', cmd, ['findre', 'splitMarkerExtr', 'isignorecase'], () => {
      getSelection((text) => {
        ...
        text.replace(new RegExp(prefs.findre.value, 'g' + (prefs.isignorecase.value ? 'i' : '')), () => {
          const record = [];
          let i = 1;
          if (mainArguments && mainArguments.length) {
            if (typeof mainArguments[1] === 'number') {
              foundtext.push(mainArguments[0]);
            } else {
              while (typeof mainArguments[i] !== 'number') {
                record.push(mainArguments[i]);
                i++;
              }
              foundtext.push(record.join(spliter));
            }
          }
          return mainArguments ? mainArguments[0] : '';
        });
      ...
      changeSelectionRegEx(new RegExp(findtext, (prefs.isall.value ? 'g' : '') +
        ...
        (startValue !== undefined) && (stepValue !== undefined) ?
          ...
          () => {
            ...
            for (let i = 1; i < mainArguments.length - 2; i++) {
              out = out.replace(new RegExp('\\$' + i, 'g'), mainArguments[i]);
            }
